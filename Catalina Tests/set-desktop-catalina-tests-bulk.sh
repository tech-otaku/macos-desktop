...
rm "$location"/Catalina\ Tests/set-desktop-catalina-tests.txt
...
for (( i=1;i<=$TIMES;i++ )); do 
	RANDNUM=$((1 + RANDOM % $(wc -l <  "$location"/Catalina\ Tests/desktop-images.txt)))
	SELECTION=$(sed ''$RANDNUM'!d' "$location"/Catalina\ Tests/desktop-images.txt)
	#ls "$SELECTION"
	bash "$location"/Catalina\ Tests/set-desktop-catalina-tests.sh "$SELECTION" $i $TIMES
done
